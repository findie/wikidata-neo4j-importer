...
const deadLockRetrier = function(session, command, params, _then, _catch, _retry) {
  session
    .run(command, params)
    .then(
      _ => _then(),
      (err) => {
        ...
        console.log(clc.bold.red(`Deadlock detected and averted (retry: ${_retry || 0}/5), waiting ${wait}ms! ${err.message}`));
        return setTimeout(() => deadLockRetrier(session, command, params, _then, _catch, (_retry || 0) + 1), wait);
      });
};
